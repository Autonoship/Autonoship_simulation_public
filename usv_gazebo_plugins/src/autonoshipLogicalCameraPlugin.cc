...
void ROSLogicalCameraPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->imagePub = this->rosnode->advertise<usv_gazebo_plugins::LogicalCameraImage>(imageTopic_ros, 1, true);
  ...
  gzdbg << "initialized transfrom broadcaster" << "\n";

  // initialize rotary speed of radar

  w_ = _sdf->GetElement("rpm")->Get<double>();
  w_ = 3.1415927 * 2 * w_ / 60;  

  P_ = _sdf->GetElement("P")->Get<double>();
  I_ = _sdf->GetElement("I")->Get<double>();
  D_ = _sdf->GetElement("D")->Get<double>();
  // this->CurrentRpmPub = this->rosnode->advertise<std_msgs::Float64>("CurrentRadarRpm", 1, true);

  w_sub_ = this->rosnode->subscribe("rpm", 1, &ROSLogicalCameraPlugin::UpdateRpm, this );
  gzdbg << "setting the RPM of radar as: " << w_ << "\n";

  // this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&ROSLogicalCameraPlugin::UpdateChild, this));

  // Store the pointers to the joints
  if(this->model->GetJoint("radar_rotation_link_joint"))
  {
    this->jointRadar_ = this->model->GetJoint("radar_rotation_link_joint");
    gzdbg << "Found radar joint: " << this->model->GetJoint("radar_rotation_link_joint")->GetName() << "\n";
  }
  ...
  this->model->GetJointController().reset(new physics::JointController(this->model));
...
void ROSLogicalCameraPlugin::OnImage(ConstLogicalCameraImagePtr &_msg)
{
  ...
  // this->CurrentRpmPub.publish(this->jointRadar_->GetVelocity(5)/2.0/3.1415926*60.0);
  this->model->GetJointController()->Update();
...
void ROSLogicalCameraPlugin::AddNoise(ignition::math::Pose3d & pose)
{
  ...
  {
    double newX = this->noiseModels["POSITION_NOISE"]->Apply(pose.Pos().X());
    double newY = this->noiseModels["POSITION_NOISE"]->Apply(pose.Pos().Y());
    double newZ = this->noiseModels["POSITION_NOISE"]->Apply(pose.Pos().Z());
    ignition::math::Vector3d newPos = ignition::math::Vector3d(newX, newY, newZ);
    pose.Set(newPos, rot);
  ...
  if (this->noiseModels.find("ORIENTATION_NOISE") != this->noiseModels.end())
  {
    ...
    // pose.rot *= pert;
    pose.Set(pose.Pos(), pose.Rot() * pert);
  }
}
...
void ROSLogicalCameraPlugin::AddModelToMsg(
  ...
  usv_gazebo_plugins::LogicalCameraImage & imageMsg)
{
  usv_gazebo_plugins::Model modelMsg;
...
void ROSLogicalCameraPlugin::UpdateRpm(const std_msgs::Float64 &w)
{
    w_ = 3.1415927 * 2 / 60.0 * w.data;
    ROS_INFO_STREAM("Receiving radar rpm: " << w.data);
    // this->jointRadar_->SetVelocity(0, w_);
    // this->model->GetLink("radar_link")->SetAngularVel({0, 0, w_});
    ...
    // ROS_INFO_STREAM("radar link rpm: " << this->model->GetLink("radar_link")->GetWorldAngularVel()/2.0/3.1415926*60.0);
    // ROS_INFO_STREAM("radar link linear velocity: " << this->model->GetLink("radar_link")->GetWorldLinearVel()/2.0/3.1415926*60.0);
}
