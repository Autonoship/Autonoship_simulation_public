...
public:
    ...
    bool createInstance(const double startTime)
    {
        ...
        // gzmsg << "fmuHandle: " << fmuHandle << std::endl;
        jmstatus = fmi2_import_instantiate(fmuHandle, instanceName.c_str(), fmi2_cosimulation, NULL, fmi2_true);
        // gzmsg << "fmi2_import_instantiate finished" << std::endl;
        ...
        fmistatus = fmi2_import_setup_experiment(fmuHandle,
                                                ...
                                                fmi2_false, 0.0);
        gzmsg << "fmi2_import_setup_experiment" << std::endl;
        ...
        gzmsg << "fmi2_import_enter_initialization_mode" << std::endl;
        ...
        gzmsg << "fmi2_import_exit_initialization_mode" << std::endl;
...
{
    gzmsg << "loading the FMU file" << std::endl;
    ...
    // m_pimpl->context = fmi_import_allocate_context(NULL);
    ...
    // gzmsg << tmpPath.c_str() << std::endl;
    // gzmsg << "created FMU handle" << std::endl;
    ...
    m_pimpl->callBackFunctions.stepFinished = NULL;
    m_pimpl->callBackFunctions.componentEnvironment = NULL; // m_pimpl->fmuHandle;

    jm_status_enu_t status = fmi2_import_create_dllfmu(m_pimpl->fmuHandle, fmi2_fmu_kind_cs, &m_pimpl->callBackFunctions);

    // gzmsg << "created environment" << std::endl;
    ...
    // m_pimpl->instanceName = "ShipWareDynamics";
    // gzmsg << "creating instance: " << instanceName << std::endl;
    bool createInstance = m_pimpl->createInstance(startTimeInSeconds);

    // gzmsg << "created instance" << std::endl;
...
bool FMUCoSimulation::getTunableParameterRefs(const std::vector< std::string >& tunableParameterNames,
                                           std::vector< fmi2_value_reference_t >& tunableParameterReferences)
{
   tunableParameterReferences.resize(tunableParameterNames.size());

   for(size_t i=0; i < tunableParameterNames.size(); i++)
   {
       fmi2_import_variable_t* var = fmi2_import_get_variable_by_name(m_pimpl->fmuHandle, tunableParameterNames[i].c_str());
       if (!var) {
           gzerr << "gazebo_fmi: impossible to find parameter of name \"" << tunableParameterNames[i] << "\" in FMU." << std::endl;
           return false;
       }
       if (fmi2_causality_enu_parameter != fmi2_import_get_causality(var)) {
           gzerr << "gazebo_fmi: found parameter of name " << tunableParameterNames[i] << " in FMU, but causality is not tunable." << std::endl;
           return false;
       }
       if (fmi2_base_type_real != fmi2_import_get_variable_base_type(var)) {
           gzerr << "gazebo_fmi: found parameter of name " << tunableParameterNames[i] << " in FMU, but type is real." << std::endl;
           return false;
       }
       tunableParameterReferences[i] = fmi2_import_get_variable_vr(var);
   }
   return true;
}
...
bool FMUCoSimulation::getOutputVariables(const std::vector< fmi2_value_reference_t >& outputVariableReferences, std::vector< double >& outputVariables)
{
    outputVariables.resize(outputVariableReferences.size());

    fmi2_status_t fmistatus = fmi2_import_get_real(m_pimpl->fmuHandle, outputVariableReferences.data(), outputVariables.size(), outputVariables.data());
...
bool FMUCoSimulation::setInputVariables(const std::vector< fmi2_value_reference_t >& inputVariableReferences, const std::vector< double >& inputVariables)
{
    ...
    fmi2_status_t fmistatus = fmi2_import_set_real(m_pimpl->fmuHandle, inputVariableReferences.data(), inputVariables.size(), inputVariables.data());
...
bool FMUCoSimulation::setTunableParameters(const std::vector< fmi2_value_reference_t >& tunableParameterReferences, const std::vector< double >& tunableParameters)
{
    if (tunableParameterReferences.size() != tunableParameters.size()) {
         gzerr << "gazebo_fmi: FMUCoSimulation::setParameters argument size mismatch." << std::endl;
        return false;
    }

    fmi2_status_t fmistatus = fmi2_import_set_real(m_pimpl->fmuHandle, tunableParameterReferences.data(), tunableParameters.size(), tunableParameters.data());
